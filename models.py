...
from examples.early_network import EarlyNetBranchy1
...
def create_discriminator():
    discriminators = []
    discriminator1 = EarlyNetBranchy1()
    discriminators.append(discriminator1)
    return nn.ModuleList(discriminators)


class Darknet(nn.Module):
    # YOLOv3 object detection model

    def __init__(self, cfg, img_size=(416, 416), light_weight=False, arc='default'):
        ...
        if light_weight:
            self.discriminators = create_discriminator()
    ...
    def forward(self, x, var=None):
        ...
        for i, (mdef, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if mtype in ['convolutional', 'upsample', 'maxpool']:
                x = module(x)
                if i == 1 and self.light_weight:
                    prons = F.softmax(self.discriminators[0](x), dim=1)
                    max_index = np.argmax(prons.detach().cpu().numpy())
                    if max_index != 0:  # True: heavy-weight model processing
                        return None, None
    ...
    for i, (mdef, module) in enumerate(zip(self.module_defs[:cutoff], self.module_list[:cutoff])):
        if mdef['type'] == 'convolutional':
            ...
            if mdef['batch_normalize']:
                ...
                if os.path.basename(file) == 'yolov3.weights' or os.path.basename(file) == 'yolov3-tiny.weights' or os.path.basename(
