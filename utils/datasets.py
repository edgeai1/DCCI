...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=True, image_weights=False,
        ...
        with open(path, "r") as f:
            self.img_files = [
                x.replace("/", os.sep)
                for x in f.read().splitlines()  # os-agnostic   f.read()读取所有的文件
        ...
        if cache_labels or image_weights:  # cache labels for faster training
            ...
            for i, file in enumerate(pbar):
                ...
                pbar.desc = (
                        "Reading labels (%g found, %g missing, %g empty for %g images)"
                        % (nf, nm, ne, n)
                )
            assert (
                    nf > 0
        ...
        if cache_images and augment:  # if training.
            for i in tqdm(
                    range(min(len(self.img_files), 10000)), desc="Reading images"
            ):  # max 10k images
...
def augment_hsv(img, hgain=0.5, sgain=0.5, vgain=0.5):
    ...
    img_hsv = (
        (cv2.cvtColor(img, cv2.COLOR_BGR2HSV) * x.reshape((1, 1, 3)))
            .clip(None, 255)
            .astype(np.uint8)
    )
...
def letterbox(img, new_shape=416, color=(128, 128, 128), mode="square", interp=cv2.INTER_AREA):
...
def random_affine(img, targets=(), degrees=10, translate=0.1, scale=0.1, shear=10):
...
def convert_images2bmp():
    ...
    for label_path in ["../coco/trainvalno5k.txt", "../coco/5k.txt"]:
        with open(label_path, "r") as file:
            lines = file.read()
        lines = (
            lines.replace("2014/", "2014bmp/")
                .replace(".jpg", ".bmp")
                .replace("/Users/glennjocher/PycharmProjects/", "../")
        )
