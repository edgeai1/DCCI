    ...
    for i, module_def in enumerate(module_defs):
        if module_def["type"] == "convolutional":
            if module_def["batch_normalize"] == "1":
                ...
                Conv_idx.append(i)
            if (
                    module_defs[i + 1]["type"] == "maxpool"
                    and module_defs[i + 2]["type"] == "route"
            ):
                ignore_idx.add(i)
...
def gather_bn_weights(module_list, prune_idx):
    ...
    for idx in prune_idx:
        size_list.append(module_list[idx][1].weight.data.shape[0])

    bn_weights = torch.zeros(sum(size_list))
    index = 0
    for idx, size in zip(prune_idx, size_list):
        bn_weights[index: (index + size)] = (
            module_list[idx][1].weight.data.abs().clone()
        )
        index += size
...
def get_layers(module_list, prune_idx):
    size_list = [module_list[idx] for idx in prune_idx]
    return size_list
        ...
        for module_def in module_defs:
            ...
            for key, value in module_def.items():
                ...
                if key != "type":
                    if key == "anchors":
                        value = ", ".join(
                            ",".join(str(int(i)) for i in j) for j in value
                        )
                    f.write(f"{key}={value}\n")
            f.write("\n")
    return cfg_file
    ...
    def updateBN(sr_flag, module_list, s, prune_idx, epoch, idx2mask=None, opt=None):
        if sr_flag:
            ...
            if idx2mask:
                for idx in idx2mask:
                    bn_module = module_list[idx][1]
                    bn_module.weight.grad.data.sub_(
                        0.99
                        * s
                        * torch.sign(bn_module.weight.data)
                        * idx2mask[idx].cuda()
                    )
...
def init_weights_from_loose_model(
    ...
    for idx in CBL_idx:
        ...
        compact_bn.running_mean.data = loose_bn.running_mean.data[
            out_channel_idx
        ].clone()
    ...
    for idx in Conv_idx:
        ...
        input_mask = get_input_mask(loose_model.module_defs, idx, CBLidx2mask)
        in_channel_idx = np.argwhere(input_mask)[:, 0].tolist()
        compact_conv.weight.data = loose_conv.weight.data[
                                   :, in_channel_idx, :, :
                                   ].clone()
        ...
        if model_def["type"] == "convolutional":
            ...
            if len(from_layers) == 1:
                activation = activations[
                    i + from_layers[0] if from_layers[0] < 0 else from_layers[0]
                ]
                ...
                actv2 = activations[
                    i + from_layers[1] if from_layers[1] < 0 else from_layers[1]
                ]
